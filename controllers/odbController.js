...
export const calculateProcessingOrderCount = async (req, res) => {
    try {
        ...
        return res.status(200).json({ result }); // SEND CORRECTLY CALCULATED VALUES TO THE INITIAL FUNCTION
    } catch (error) {
        console.log(error);
        const result = {
            processingCount: 0,
        ...
        return res.status(500).json({ result }); // SEND 0s WHEN ERROR OCCURS
    }
}


// LOW STOCK MATERIAL COUNT CALCULATING FUNCTION - LOCALLY CALLING (WORKING)
export const calculateLowStockMaterialCount = async (req, res) => {
    try {
...
export const calculateOrderCount = async (req, res) => {
    try {
        // CALCULATE THE END DATE (TODAY)
        const today = moment().toDate();
        // CALCULATE THE START DATE (7 DAYS BEFORE TODAY)
...
export const findIncomeTotal = async (req, res) => {
    try {
        // CALCULATE THE END DATE (TODAY)
        const today = moment().toDate();
        // CALCULATE THE START DATE (7 DAYS BEFORE TODAY)
...
export const getRecentOrders = async (req, res) => {
    try {
        ...
        return res.status(200).json({ recentOrders }); // SEND RECENT 5 RECORDS TO THE INITIAL FUNCTION
    } catch (error) {
        console.log(error);
        return res.status(500).json({ message: error }); // SEND NULL WHEN ERROR OCCURS
    }
}
...
export const getWeeklyPerformance = async (req, res) => {
    try {
        ...
        return res.status(200).json({ weeklyPerformance });
    } catch (error) {
        console.log(error);
        const weeklyPerformance = {
        ...
        return res.status(500).json({ weeklyPerformance });
    }
}